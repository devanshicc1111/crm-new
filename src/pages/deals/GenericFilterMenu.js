...
import { ClickAwayListener, IconButton, MenuItem, MenuList, Paper, Popper, Tooltip } from '@mui/material'
...
const GenericFilterMenu = props => {
  ...
  const handleSortType = arr =>
    arr.map(item => (
      <MenuItem className={classes.menuStyling} key={item.label} onClick={e => handleChangeSortOrder(e, item.value)}>
        <Done className={sortObj.type !== item.value ? classes.displayNone : classes.doneIcon} />
        {item.label}
      </MenuItem>
    ))
              ...
              <ClickAwayListener onClickAway={handleClose}>
                <MenuList id='menu-list-grow' onKeyDown={handleListKeyDown}>
                  ...
                  {props.SortValueTypes?.map((item, i) => (
                    ...
                    >
                      <Done className={sortObj.title?.name !== item.name ? classes.displayNone : classes.doneIcon} />
                      {item.label}
                    </MenuItem>
                  ))}
                  <div style={{ borderTop: '1px solid #ddd' }}>
                    ...
                    {sortObj.title?.type === 'date' ? handleSortType(dateOrder) : handleSortType(strOrder)}
                  </div>
                </MenuList>
              </ClickAwayListener>
            </Paper>
